    ...
    public <T> JsonRpcResponse<T> request(JsonRpcRequest request, Class<T> responseType) {
        ...
        try (CloseableHttpResponse response = httpClient.execute(post)) {
            ...
            if (statusCode < 200 || statusCode >= 300) {
                ...
                throw new HttpException(statusCode, responseBody, "Failed : HTTP error code : " + statusCode);
            }

            String responseBody = EntityUtils.toString(response.getEntity());
            JsonRpcResponse<T> rpcResponse = objectMapper.readValue(responseBody, objectMapper.getTypeFactory().constructParametricType(JsonRpcResponse.class, responseType));
