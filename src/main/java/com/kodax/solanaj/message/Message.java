...
public class Message {
    ...
    public static Message compile(PublicKey payer, List<Instruction> instructions, String recentBlockHash) {
        List<PublicKey> accountKeys = Stream.concat(
                        Stream.of(payer),
                        instructions.stream()
        ...
        int numRequiredSignatures = (int) instructions.stream()
                .flatMap(instruction -> instruction.getKeys().stream())
                .filter(AccountMeta::isSigner)
                .distinct()
                .count();

        int numReadonlySignedAccounts = (int) instructions.stream()
                .flatMap(instruction -> instruction.getKeys().stream())
                .filter(meta -> meta.isSigner() && !meta.isWritable())
                .distinct()
                .count();

        int numReadonlyUnsignedAccounts = (int) instructions.stream()
                .flatMap(instruction -> instruction.getKeys().stream())
                .filter(meta -> !meta.isSigner() && !meta.isWritable())
                .distinct()
                .count();

        MessageHeader header = MessageHeader.of(
    ...
    public byte[] serialize() {
        ...
        for (Instruction instruction : instructions) {
            ...
            List<Integer> accounts = instruction.getKeys().stream()
            ...
            ByteBuffer instructionBuffer = ByteBuffer.allocate(1 + keyIndicesCount.length + accounts.size() + dataCount.length + data.length);
